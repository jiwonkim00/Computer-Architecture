...
int nibbleReverse(int x) {
    ...
    mask1 = (0x0F) | (0x0F << 8) | (0x0F << 16) | (0x0F << 24);  // mask1 = 0x0F0F0F0F

    mask2 = mask1 << 4; // mask2 = 0xF0F0F0F0

    // Separating the nibbles
    reversed = ((x & mask1) << 28) | ((x & mask2) << 24) | ((x & mask1) << 8) | ((x & mask2) << 4); //first half
    reversed = reversed |= ( ((x & mask1) >> 4) | ((x & mask2) >> 8) | ((x & mask1) >> 12) ); //second half except for the lsb
    lsb = ((x & mask2) >> 28) & 0xf;
    reversed |= lsb;
...
int numZerosFirst(int x) {
    ...
    result = mask; /* Start count with mask (number of bits we’ve handled) */
